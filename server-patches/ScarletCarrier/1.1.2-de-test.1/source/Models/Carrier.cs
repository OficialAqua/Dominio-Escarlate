...
internal class Carrier {
  ...
  public void Dismiss() {
    if (Busy || !ServantEntity.Exists()) return;
  ...
  private void SetName(string playerName) {
    if (!CoffinEntity.Exists() || !CoffinEntity.Has<ServantCoffinstation>()) return;
  ...
  public void RecreateCoffin() {
    ...
    if (!ServantEntity.Exists()) {
      Log.Warning($"Cannot recreate coffin without a valid servant for player {Owner.Name}");
      return;
    }
  ...
  private void BindCoffinServant() {
    if (!ServantEntity.Exists() || !CoffinEntity.Exists()) return;
  ...
  public void StopFollow() {
    ...
    if (ServantEntity.Exists() && ServantEntity.Has<AiMove_Server>()) {
      StopSeekingPosition();
    }
    _positionHistory.Clear();
  }
  ...
  public void DestroyEntitiesSafely() {
    StopFollow();
    ...
    if (ServantEntity.Exists()) {
      try {
        ...
        Log.Warning($"Failed to destroy carrier servant for player {Owner.Name}: {ex.Message}");
      }
    }
    ...
    if (CoffinEntity.Exists()) {
      try {
        ...
        Log.Warning($"Failed to destroy carrier coffin for player {Owner.Name}: {ex.Message}");
      }
    }
  }
